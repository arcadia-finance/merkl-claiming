...
distributor = w3.eth.contract(
    address="0x3Ef3D8bA38EBe18DB133cEc108f4D14CE00Dd9Ae", abi=distributor_abi
)
arcadia_factory = w3.eth.contract(
    address="0xDa14Fdd72345c4d2511357214c5B89A919768e59", abi=arcadia_factory_abi
)
    ...
    for i in range(0, amount_of_accounts, 100):
        ...
        calls = [
            Call(
                ...
                ["allAccounts(uint256)(address)", index],
                [(str(index), from_value)],
            )
            for index in batch
        ]
    ...
    for i in range(0, len(accounts), 100):
        ...
        calls = [
            Call(
                ...
                ["operators(address,address)(bool)", account, CLAIMER],
                [(account, None)],
            )
        ...
        multicall = Multicall(calls, _w3=w3)
        results = await multicall.coroutine()
        is_operator_set.update(
            {account: True if results[account] == 1 else False for account in batch}
        )
...
async def get_accounts_without_operator():
    ...
    accounts_to_set_operator = [
        account for account, value in is_operator_set.items() if value is False
    ]
